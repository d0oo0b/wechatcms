...
<div class="cLineB">
  <h4>添加楼盘相册</h4><a href="javascript:history.go(-1);return false;" class="right btnGrayS vm" style="margin-top:-27px">返回</a>
 </div>
    ...
    <div class="msgWrap bgfc">
	  <form class="form" method="post" action="" target="_top" >
		<table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%">
			<tbody>
				<tr>
				  <th valign="top"><span class="red">*</span>经选择相册名称：</th>
				  <td>
					  <select id="poid" name="poid" class="input-medium" schoolSet>
                            <option value="0">请选择相册</option>
                                <?php if(is_array($photo)): $i = 0; $__LIST__ = $photo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php if($vo['id'] == $album['poid']): ?>selected="selected"<?php endif; ?>><?php echo ($vo['title']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                          </select>
                     <a href="<?php echo U('Photo/add',array('token'=>$token,'poid'=>$poid['poid']));?>" class="btn"><i class="icon-plus"></i>添加相册</a></td>
				</tr>


				<th>&nbsp;</th>
				  <?php if($album['id'] != ''): ?><input type="hidden" name="id" value="<?php echo ($album['id']); ?>" /><?php endif; ?>
					<td>
